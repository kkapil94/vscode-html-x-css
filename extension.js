...
function generateCssFromHtml() {
  if (vscode.window.activeTextEditor) {
    ...
    vscode.window.activeTextEditor.selections.forEach((element) => {
      let selectedText =
        vscode.window.activeTextEditor.document.getText(element);
    ...
    if (allSelectedText) {
      ...
      if (allClass.isNotEmpty()) {
        let clipboard = "";
        let config = vscode.workspace.getConfiguration("vscode-html-x-css");

        allClass
          .unique()
          .except(config.get("ignore"))
          .each((item) => {
        ...
        if (!config.get("disableNotification")) {
          vscode.window.showInformationMessage(
            "CSS boilerplate copied to clipboard."
          );
        }
      }
    }
